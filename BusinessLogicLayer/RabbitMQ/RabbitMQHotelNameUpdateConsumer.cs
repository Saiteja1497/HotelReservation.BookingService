...
{
    public class RabbitMQHotelNameUpdateConsumer : IDisposable, IRabbitMQHotelNameUpdateConsumer
    {

        private readonly IConfiguration _configuration;
        private IChannel _channel;
        private IConnection _connection;
        private readonly ILogger<RabbitMQHotelNameUpdateConsumer> _logger;
        private readonly IDistributedCache _cache;
        public RabbitMQHotelNameUpdateConsumer(IConfiguration configuration, 
            ILogger<RabbitMQHotelNameUpdateConsumer> logger,
        ...
        {
            //string routingKey = "hotel.update.name";
            var headers = new Dictionary<string, object>() {
                    {"x-match","all" },
                    {"event","hotel.update" },
                    {"RowCount",1 }
                };
            string queueName = "bookings.hotel.update.name.queue";
            ...
            consumer.ReceivedAsync += async (sender, args) =>
            {
                ...
                try
                {
                    if (message != null)
                    {
                        HotelDTO? HotelDTO = JsonSerializer.Deserialize<HotelDTO>(message);
                        if (HotelDTO != null)
                        {
                            await HandleHotelUpdation(HotelDTO);
                        }
                    }
                }
                catch (Exception ex)
        ...
        private async Task HandleHotelUpdation(HotelDTO hotelDTO)
        {
            _logger.LogInformation($"Updating bookings for HotelID: {hotelDTO.HotelID} with new name: {hotelDTO.HotelName}");

            string hotelJson = JsonSerializer.Serialize(hotelDTO);
            string cacheKeyToWrite = $"Hotel_{hotelDTO.HotelID}";
